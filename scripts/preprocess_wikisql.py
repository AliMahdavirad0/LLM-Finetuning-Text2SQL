...
def extract_if_needed(tar_path: Path, extract_dir: Path) -> None:
    ...
    for p in extract_dir.rglob("train.jsonl"):
        return
...
def find_data_dir(extract_dir: Path) -> Path:
    ...
    for p in extract_dir.rglob("train.jsonl"):
        return p.parent
    raise FileNotFoundError("Could not find train.jsonl after extraction.")
...
def write_split(rows_iter, out_path: Path, limit: int):
    ...
    with out_path.open("w", encoding="utf-8") as f:
        for r in rows_iter:
            table = r["table"]
            table_name = (table.get("name") or table.get("id") or "table").strip() or (table.get("id") or "table")
            headers = table["header"]

            schema = build_schema(table_name, headers)
            gold_sql = build_sql(table_name, headers, r["sql"])

            rec = {"schema": schema, "question": r["question"], "sql": gold_sql}
...
def main():
    ...
    train_main = data_dir / "train.jsonl"
    train_tables = data_dir / "train.tables.jsonl"
    dev_main = data_dir / "dev.jsonl"
    dev_tables = data_dir / "dev.tables.jsonl"

    write_split(iter_examples(train_main, train_tables), out_dir / "wikisql_train.jsonl", args.limit_train)
    write_split(iter_examples(dev_main, dev_tables), out_dir / "wikisql_val.jsonl", args.limit_val)

    print("Done.")
    print(f"- {out_dir / 'wikisql_train.jsonl'}")
    print(f"- {out_dir / 'wikisql_val.jsonl'}")
